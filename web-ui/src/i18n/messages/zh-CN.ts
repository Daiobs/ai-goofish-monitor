import zhCNExtra from '@/i18n/messages/zh-CN-extra'

const zhCN = {
  app: {
    name: '闲鱼智能监控',
  },
  locale: {
    switchLabel: '切换语言',
    zh: '中文',
    en: 'English',
  },
  routes: {
    login: '登录',
    dashboard: '监控概览',
    tasks: '任务管理',
    accounts: '账号管理',
    results: '结果查看',
    logs: '运行日志',
    settings: '系统设置',
  },
  common: {
    cancel: '取消',
    close: '关闭',
    delete: '删除',
    edit: '编辑',
    skipToContent: '跳转到主内容',
    save: '保存',
    saving: '保存中...',
    refresh: '刷新',
    loading: '加载中...',
    error: '出错了!',
    enabled: '已启用',
    disabled: '已停用',
    running: '运行中',
    idle: '空闲',
    testing: '测试中...',
    empty: '暂无',
    unnamed: '未命名',
    unknown: '未知',
    yes: '是',
    no: '否',
    all: '不限',
    ai: 'AI',
    keyword: '关键词',
    active: '已启用',
    inactive: '未启用',
  },
  time: {
    justNow: '刚刚',
    minutesAgo: '{count} 分钟前',
    hoursAgo: '{count} 小时前',
    daysAgo: '{count} 天前',
    scheduled: '未计划',
    upcoming: '即将触发',
    countdownDays: '{days}天 {hours}时 {minutes}分',
    countdownHours: '{hours}时 {minutes}分 {seconds}秒',
    countdownMinutes: '{minutes}分 {seconds}秒',
    countdownSeconds: '{seconds}秒',
  },
  header: {
    goHome: '返回首页',
    openAccounts: '打开账号管理',
    openNavigation: '打开导航菜单',
    openNotifications: '打开通知设置',
    openPrompts: '打开 Prompt 管理',
    searchUnavailable: '任务搜索仅在监控概览页可用...',
    accountManagement: '账号管理',
  },
  sidebar: {
    dashboard: '监控概览',
    tasks: '任务管理',
    accounts: '账号管理',
    results: '结果查看',
    logs: '运行日志',
    settings: '系统设置',
    systemStatus: '系统状态',
    backendConnected: '后端实时已连接',
    backendConnecting: '后端连接中',
  },
  login: {
    title: '系统登录',
    description: '请输入您的管理员凭证以继续',
    username: '用户名',
    password: '密码',
    submit: '登录',
    submitting: '登录中...',
    errors: {
      missingCredentials: '请输入用户名和密码',
      invalidCredentials: '登录失败：用户名或密码错误',
      unexpected: '登录过程中发生错误',
    },
  },
  dashboard: {
    title: '监控概览',
    description: '这里展示任务、结果与最近活动的真实汇总。',
    createTask: '开始新监测',
    stats: {
      activeTasks: '活动任务',
      scannedItems: '已扫描商品',
      recommendedItems: '已发现推荐',
      monitoredTasks: '监测任务',
      runningCount: '运行中 {count} 个',
      resultFiles: '结果文件 {count} 个',
      recommendedBreakdown: 'AI {ai} / 关键词 {keyword}',
      showAllTasks: '当前展示全部任务',
    },
    focus: {
      defaultTitle: '价格走势洞察',
      empty: '当前没有可展示的结果文件，先创建或运行任务。',
      missingKeyword: '未配置关键词',
      meta: '关键词：{keyword}，当前累计 {count} 条结果样本。',
      latestUpdate: '最近更新 {time}',
      waiting: '等待真实结果',
      loading: 'Dashboard 数据加载中...',
      noResults: '当前还没有结果文件，先去创建任务或运行已有任务。',
      currentMedian: '当前中位数：',
      historyMin: '历史最低价：',
      historyMax: '历史最高价：',
    },
    activity: {
      title: '实时动态',
      empty: '当前没有可展示的动态。',
      viewAllLogs: '查看全部日志',
    },
    suggestion: {
      sectionTitle: 'AI 智能策略',
      firstTaskTitle: '开始首个真实监测任务',
      firstTaskDescription: '当前没有可优化的任务，先创建一个真实监测任务，再回来查看趋势和推荐。',
      firstTaskAction: '去任务页创建',
      improveValueTitle: '把高价值任务再压榨一点',
      improveValueDescription: '“{task}”已经出现真实推荐，建议提高页数并聚焦新发布商品。',
      improveHitRateTitle: '先提高监测命中率',
      improveHitRateDescription: '“{task}”还没看到推荐结果，建议扩大搜索页数并优先抓最新发布。',
      openTaskAction: '带推荐参数去任务页',
    },
  },
  results: {
    title: '结果查看',
    filters: {
      loadingTaskNames: '加载任务名称...',
      noResults: '暂无结果，请先运行任务',
      chooseResult: '请选择任务结果',
      taskNameLabel: '任务名称：{task}',
      legacyTaskNameLabel: '旧配置：{keyword}',
      sortByCrawlTime: '按爬取时间',
      sortByPublishTime: '按发布时间',
      sortByPrice: '按价格',
      sortByKeywordHits: '按命中数',
      desc: '降序',
      asc: '升序',
      aiOnly: '仅看AI推荐',
      keywordOnly: '仅看关键词推荐',
      includeHidden: '显示已屏蔽结果',
      manageBlacklist: '黑名单规则',
      exportCsv: '导出 CSV',
      deleteResult: '删除结果',
      noResultSelected: '请先选择结果文件',
      noResultToDelete: '暂无可删除的结果',
      noResultToExport: '暂无可导出的结果',
      resultDeleted: '结果已删除',
      blacklistSaved: '黑名单规则已保存',
      blacklistSaveFailed: '保存黑名单规则失败',
      deleteFailed: '删除结果失败',
      deleteDialogTitle: '删除任务结果',
      deleteDialogWithTask: '确定删除任务结果「{task}」吗？此操作不可恢复。',
      deleteDialogFallback: '确定删除该任务结果吗？此操作不可恢复。',
      confirmDelete: '确认删除',
      blacklistDialogTitle: '编辑黑名单规则',
      blacklistDialogDescription: '命中这些规则的商品会从结果页、导出和价格走势里排除，但原始结果文件仍会保留。',
      blacklistRulesLabel: '黑名单关键词',
      blacklistRulesHint: '每行一个关键词，也支持逗号分隔。默认不区分大小写。需要一条同时匹配多种写法时，可用高级规则，例如：re:\\b(pm|pro[\\s-]?max)\\b',
      blacklistRulesPlaceholder: '示例：intel\npro 13\nre:\\b(pm|pro[\\s-]?max)\\b',
      confirmBlacklistSave: '保存规则',
    },
    decisionQueue: {
      title: '决策队列',
      description: '按 AI 分类与可比性查看候选商品，优先处理最值得人工确认的结果。',
      currentViewCount: '当前视图 {count} 条',
      loadingSummary: '正在汇总任务结果...',
      summaryLabel: '任务结果整体摘要',
      emptyWorthViewing: '当前没有 AI 推荐且可比的独立目标商品',
      tabs: {
        worth_viewing: '值得查看',
        comparable_targets: '可比目标',
        bundles: '套装',
        excluded: '已排除',
        ai_issues: 'AI 异常',
      },
      summary: {
        all: '全部',
        targetOnly: '独立目标',
        bundles: '套装',
        notTarget: '非目标',
        uncertain: '待确认',
        comparable: '可比',
        excluded: '已排除',
        aiRecommended: 'AI 推荐',
        aiNotRecommended: 'AI 不推荐',
        aiIssues: 'AI 异常',
      },
    },
    grid: {
      loading: '正在加载结果...',
      empty: '没有找到符合条件的商品记录。',
    },
    insights: {
      defaultTitle: '价格走势洞察',
      subtitle: '仅使用 AI 确认的独立目标商品和明确单件价格，计算可比均价、近期波动和价格带。',
      legacySubtitle: '兼容模式按关键词快照统计，尚未经过 AI 商品类别与单价可比性筛选。',
      currentAvg: '当前可比样本均价',
      historyAvg: '历史可比均价',
      currentMin: '当前可比最低价',
      legacyCurrentAvg: '当前关键词样本均价',
      legacyHistoryAvg: '历史关键词均价',
      legacyCurrentMin: '当前关键词最低价',
      sampleCount: '样本 {count} 条',
      uniqueItems: '唯一商品 {count} 个',
      highestPrice: '最高 ¥{price}',
      noRange: '暂无范围',
      noSnapshot: '尚未形成市场快照',
      latestSnapshot: '最近更新于 {time}',
      trendReading: '价格统计不包含非目标商品、组合套装、其他型号或价格单位含糊的候选商品。',
      legacyTrendReading: '这些价格来自关键词兼容样本，解读时需要人工确认商品类别和标价单位。',
      comparisonScope: '纳入 {comparable} 条可比商品，排除 {excluded} 条非可比候选。',
      keywordScope: '当前按关键词兼容口径统计 {count} 条商品。',
      snapshotCount: '{count} 条',
      currentMedian: '当前中位数：',
      historyMin: '历史最低价：',
      historyMax: '历史最高价：',
    },
    chart: {
      avgPrice: '均价',
      medianPrice: '中位数',
      noTrend: '暂无可绘制的趋势数据',
    },
    card: {
      curated: '精选',
      strongRecommend: '强烈推荐',
      notRecommended: '不建议购买',
      aiFailed: 'AI分析失败',
      aiCompleted: 'AI分析完成',
      aiSkipped: 'AI分析已跳过',
      aiPending: 'AI分析等待中',
      aiStatus: 'AI 状态',
      pending: '待观察',
      analyzing: 'AI 正在分析该商品的潜在价值...',
      analysisReason: '结论理由',
      collapse: '收起详情',
      expand: '阅读完整理由',
      marketAvg: '目标商品可比均价',
      keywordMarketAvg: '关键词样本均价',
      historicalLow: '历史低位',
      targetOnly: '目标：独立目标商品',
      targetBundle: '含充电器的组合套装',
      notTarget: '非目标商品',
      targetUncertain: '商品类型待确认',
      targetUnclassified: '商品尚未分类',
      comparable: '计入可比价格',
      notComparable: '不计入可比价格',
      excludedFromMarket: '不参与均价',
      exclusionReason: '排除原因',
      bundleExclusion: '组合套装不进入独立目标商品决策队列。',
      nonTargetExclusion: 'AI 判定为非目标商品。',
      uncertainExclusion: '商品类型尚不能可靠确认。',
      nonComparableExclusion: '价格口径或商品状态不适合参与市场比较。',
      aiIssueExclusion: 'AI 分析尚未成功完成，暂不进入决策队列。',
      marketPosition: '市场相对位置',
      belowMedian: '低于可比市场中位数 {percent}%',
      aboveMedian: '高于可比市场中位数 {percent}%',
      atMedian: '接近可比市场中位数',
      marketPositionUnavailable: '可比市场位置暂无数据',
      excludedMarketPosition: '未纳入可比市场位置计算',
      anonymous: '匿名',
      detail: '详情',
      hidden: '已屏蔽',
      blacklisted: '规则屏蔽',
      expired: '已过期',
      block: '屏蔽',
      unblock: '取消屏蔽',
    },
  },
  logs: {
    title: '运行日志',
    task: '任务',
    selectTask: '请选择任务',
    taskRunningSuffix: '（运行中）',
    autoRefresh: '自动刷新',
    autoScroll: '自动滚动',
    clearLogs: '清空日志',
    logsCleared: '日志已清空',
    clearFailed: '清空日志失败',
    dialogTitle: '清空任务日志',
    dialogDescription: '此操作不可恢复，确定要清空当前任务日志吗？',
    confirmClear: '确认清空',
    trimmedNotice: '...日志过长已截断，仅保留最新内容...',
  },
  settings: {
    title: '系统设置',
    tabs: {
      ai: 'AI 模型',
      rotation: 'IP 轮换',
      notifications: '通知推送',
      status: '系统状态',
      prompts: 'Prompt 管理',
    },
    notifications: { saved: '通知设置已保存', saveFailed: '通知设置保存失败', testFailed: '通知测试失败' },
    ai: {
      title: 'AI 模型设置',
      description: '配置用于商品分析的大语言模型。',
      keyPlaceholder: '留空表示不修改',
      keyConfigured: '已配置，为安全起见不回显。',
      keyMissing: '未配置，为安全起见不回显。',
      modelPreset: 'OpenAI 模型',
      modelPresetPlaceholder: '选择模型',
      modelPresetHint: 'GPT-5.6 裸别名对应 Sol；选择 Terra 或 Luna 会保存精确模型 ID。',
      customModelCurrent: '自定义：{model}',
      modelName: '精确模型 ID',
      modelNameHint: '可直接填写服务商支持的其他 OpenAI-compatible 模型 ID。',
      notSelected: '未选择模型',
      reasoningEffort: '思考等级',
      reasoningPlaceholder: '选择思考等级',
      reasoningHint: '更高等级通常提高复杂判断质量，也会增加耗时和推理 Token。',
      reasoningOptions: {
        default: '服务商默认',
        none: 'None',
        low: 'Low',
        medium: 'Medium',
        high: 'High',
        xhigh: 'XHigh',
        max: 'Max',
      },
      effectiveConfiguration: '生效组合',
      proxy: '代理地址 (可选)',
      loading: '正在加载 AI 配置...',
      testConnection: '测试连接',
      save: '保存 AI 设置',
      saved: 'AI 设置已保存',
      saveFailed: 'AI 设置保存失败',
      testSuccess: 'AI 连接测试完成',
      testFailed: 'AI 连接测试失败',
    },
    rotation: { saved: '轮换设置已保存', saveFailed: '轮换设置保存失败' },
    status: {
      title: '系统运行状态',
      refresh: '刷新状态',
      scraper: '爬虫进程',
      scraperDescription: '当前是否有任务正在执行抓取',
      env: '环境变量配置',
      envDescription: '检查 .env 配置文件中的关键项',
      loaded: '已加载',
      missing: '缺失',
      channels: '通知渠道',
      none: '无',
      fetching: '正在获取系统状态...',
    },
    prompts: {
      title: 'Prompt 管理',
      description: '在线编辑 prompts 目录下的 Prompt 文件。',
      selectFile: '选择 Prompt 文件',
      placeholder: '请选择一个 Prompt 文件...',
      none: '没有找到 Prompt 文件。',
      content: 'Prompt 内容',
      contentPlaceholder: '请选择一个 Prompt 文件进行编辑...',
      save: '保存更改',
      promptListFailed: '加载 Prompt 列表失败',
      selectPromptFile: '请选择 Prompt 文件',
      saveSuccess: 'Prompt 保存成功',
      saveFailed: 'Prompt 保存失败',
      promptContentFailed: '加载 Prompt 内容失败',
    },
  },
  ...zhCNExtra,
}

export default zhCN
